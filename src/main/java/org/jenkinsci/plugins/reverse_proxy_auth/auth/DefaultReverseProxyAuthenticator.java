...
public class DefaultReverseProxyAuthenticator
    implements ReverseProxyAuthenticator, InitializingBean, MessageSourceAware {

  private static final Logger LOGGER = Logger.getLogger(ReverseProxySecurityRealm.class.getName());

  @SuppressFBWarnings(
      value = "URF_UNREAD_PUBLIC_OR_PROTECTED_FIELD",
      justification = "It is a part of public API :(")
  ...
  public DefaultReverseProxyAuthenticator(
      String username, @CheckForNull GrantedAuthority[] authorities) {
  ...
  public ReverseProxyUserDetails authenticate(String username, String password)
    ...
    LOGGER.log(
        Level.INFO,
        "DefaultReverseProxyAuthenticator::authenticate ==> {0} to {1}",
        new Object[] {this.username, authorities});
    ...
    if (principal instanceof ReverseProxyUserDetails) {
      ...
      throw new InvalidDataAccessApiUsageException(
          "Cannot convert principal "
              + principal
              + " to ReverseProxyUserDetails. "
              + "Principal type is "
